...
contract stPENDLE is ERC4626, OwnableRoles, ReentrancyGuard { 
    ...
    mapping(address user => mapping(uint256 epoch => uint256 pendingRedemptionAmount)) public pendingRedemptionSharesPerEpoch;
    ...
    */
    ...
    function processRedemptions() external nonReentrant whenNotPaused {
        ...
        if(availableForRedemption < totalPendingRedemptions) revert InvalidRedemptionAmount(totalPendingRedemptions, availableForRedemption);
        // withdraw from voting escrow
        uint256 withdrawnAmount = votingEscrowMainchain.withdraw();
        if(withdrawnAmount < availableForRedemption) revert InvalidRedemptionAmount(withdrawnAmount, availableForRedemption);
   ...
   function name() public pure override returns (string memory) {
    ...
    function previewVeWithdraw() public view returns (uint256) {
        ...
        (bool success, bytes memory data) = address(votingEscrowMainchain).staticcall(abi.encodeWithSelector(votingEscrowMainchain.withdraw.selector));
    ...
    function _requireIsWithinRedemptionWindow() internal view {
        if (block.timestamp > vaultPosition.lastEpochUpdate + vaultPosition.preLockRedemptionPeriod) revert OutsideRedemptionWindow();
    }
